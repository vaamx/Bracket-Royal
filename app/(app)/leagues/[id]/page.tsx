...
export default async function LeaguePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-md space-y-5 p-6">
      <Link href="/leagues" className="inline-block text-sm text-white/50 hover:text-white/80">← Leagues</Link>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          <p className="text-xs font-bold tracking-[2px] text-[var(--bn-accent)]">LEADERBOARD</p>
          <h1 className="truncate text-2xl font-black">{data.league.name}</h1>
          <p className="text-xs text-white/45">{players} {players === 1 ? "player" : "players"}</p>
        </div>
      </div>
